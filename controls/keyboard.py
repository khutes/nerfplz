...
def run():
	...
	try:	
		...
		# event handling
			...
			if keys[pygame.K_w]:
				send_to_pi.messages(1, -1)
				time.sleep(0.1)				
			if keys[pygame.K_a]:
				send_to_pi.messages(2, -1)
				time.sleep(0.1)					
			if keys[pygame.K_s]:
				send_to_pi.messages(3, -1)
				time.sleep(0.1)					
			if keys[pygame.K_d]:
				send_to_pi.messages(4, -1)
				time.sleep(0.1)					
			if keys[pygame.K_UP]:
				send_to_pi.messages(5, -1)
				time.sleep(0.1)					
			if keys[pygame.K_LEFT]:
				send_to_pi.messages(6, -1)
				time.sleep(0.1)					
			if keys[pygame.K_DOWN]:
				send_to_pi.messages(7, -1)
				time.sleep(0.1)					
			if keys[pygame.K_RIGHT]:
				send_to_pi.messages(8, -1)
				time.sleep(0.1)					
			...
			for event in pygame.event.get():
				...
				if event.type == pygame.KEYUP:
					if event.key == pygame.K_d:
						send_to_pi.messages(4, 0)
				if event.type == pygame.KEYUP:
					if event.key == pygame.K_UP:
						send_to_pi.messages(5, 0)
				if event.type == pygame.KEYUP:
					if event.key == pygame.K_LEFT:
						send_to_pi.messages(6, 0)
				if event.type == pygame.KEYUP:
